...
var lastID=-1;  //存储上次id,因为id一般为正值，所以第一次设置为负值就行
...
listenerList.click = function (targetList,camera) {
	...
	function move(event) {
        ...
        if(intersects.length>0){
           if(currentClick = intersects[0].object){
               return;
           }else{
               Click =false;
               currentClick = null;
...
listenerList.hover = function (targetList,camera) {
	var lastObject,currentObject,lastEvents,currentEvents;
	...
	window.addEventListener('mousemove',function(event) {
	    ...
	    if (intersects.length > 0) {
	        ...
	        if(lastID == -1){
                ...
                lastID =  currentObject.id;
                currentEvents = getEventObj(targetList,currentObject).callback;
                
                if(!!currentEvents[0]) currentEvents[0](currentObject);
	            ...
	            if(intersects[0].id !== lastObject.id){
                  
                    if(!!lastEvents[1]){
                        lastEvents[1](lastObject);
                    }
