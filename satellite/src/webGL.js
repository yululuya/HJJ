...
var hotspot_alt1=new THREE.Object3D(), hotspot_alt2=new THREE.Object3D();
...
window.initWebGL = function(){
	...
	$(canvas).css({"font-family": "Borda"});

	var gl = canvas.getContext("webgl");
	if(!gl) {var gl = canvas.getContext("experimental-webgl");}
	var contextAttribs = gl.getContextAttributes();
	if (contextAttribs.antialias || !contextAttribs.antialias) {
		console.log("browser needs custom antialias");
		IDR.needsAntialias = true;
		...
		var effectFilm = new THREE.FilmPass(.5,.5,1000,0);
		effectFilm.renderToScreen = true;
		//composer.addPass( effectFilm );

		// use this effect to texture of the image
		var effect = new THREE.ShaderPass( THREE.RGBShiftShader );
		effect.uniforms[ 'amount' ].value = 0.007;
		...
		effect.uniforms[ 'resolution' ].value.set( 1/w, 1/h );
		effect.renderToScreen = true;
		//composer.addPass( effect );
		
		*/
	} else {
		IDR.needsAntialias = false;
	}
	...
	function render() {
		...
		if(timeline){
			...
			if(-targetRotationX < minRot) { targetRotationX = -minRot;}
			if(-targetRotationX > maxRot) { targetRotationX = -maxRot;}
	...
	function onTimeLineUpdated(){
		...
		if(r<7 && r>=1){
			...
			// 3 days of attack
			m=7; y=1996; d=Math.floor((r-6)/12) +2;
		...
		if(i!=-1 && IDR.showingEventIndex!=i){
			...
			hotspot.hide();
			setTimeout(function(){hotspot.show();},300);
			console.log(i);
			if(i==1){
				// show the alt hotspots as well
				setTimeout(function(){hotspot_alt1.show();},300);
		...
		if(d==1 && m==12 && y==1997) { d=30; y=1997; m=11; }
		...
		if(r>=282.1 && IDR.mode=="play") { 
			...
			if(!waitingToReplay){
				...
				IDR.replayTimer = setTimeout(function(){
					...
					setTimeout(function(){
						locked = false; finishedDrift =false; hadClickedTimelineHotSpot = false;
						hotspot.event_data = null;
						IDR.showingEventIndex = null;
						timeline.dial.rotation.y = 1.589;
						targetRotationX = -1.589;
						setTimeout(function(){playSpeech(); waitingToReplay = false; },500);
					},3000);

					setTimeout(function(){$("#webgl-content").addClass("show"); },4000);
	...
	function addEventListeners(){
		...
		function onWindowResize(){
		    ...
		    renderer.setSize($("#webgl-content").width(), $("#webgl-content").height());
		  	...
		  	if (window.innerWidth<window.innerHeight) { 
		  		IDR.isPortrait = true; camera.position.z = 700; 
		  		// adjust timeline
		  		if(timeline){
		  		...
		  		IDR.isPortrait = false; camera.position.z = 550;
		  		if(timeline){
			  		// adjust timeline
		...
		$("#preview").click(function(e){
			...
			//IDR.audio.playFromTo("click",0,1);
			if(IDR.mode=="play" && IDR.speechPlaying) { pauseSpeech();}
			trackEvent({'ea':{'name':'Nav','action':'Thumb','label':hotspot.event_data.title},'event':'se'}) ;
		});
		...
		/*
		$("#webgl-content").click(function(e){
			console.log("pciked up click on webgl detecor");
			if(showingDetail) { hideDetailView(); IDR.audio.playFromTo("click",0,1);}
		})
		
		$("#timeline-detector").click(function(e){
			console.log("pciked up click on timeline detecor");
			if(showingDetail) { hideDetailView(); IDR.audio.playFromTo("click",0,1);}
		})
*/
		...
		function onDocumentMouseDown(event) {
			if(!showingSignup){event.preventDefault();}
			...
			if ( intersects.length > 0 ) {
				...
				if(intersects[ 0 ].object.userData.type == 'timeline_hotspot' || intersects[0].object.parent.userData.type== 'timeline_hotspot'){
					...
					if(intersects[ 0 ].object.userData.index){
						...
						finishedDrift = false; hadClickedTimelineHotSpot = true;
		...
		function onDocumentTouchStart( event ) {
		        ...
		        if ( event.touches.length == 1 ) {
						...
						if ( intersects.length > 0 ) {
							...
							if(intersects[ 0 ].object.userData.type == 'timeline_hotspot' || intersects[0].object.parent.userData.type== 'timeline_hotspot'){document.body.style.cursor = "pointer";
								//console.log("got hotspot on timeline:" + intersects[ 0 ].object.userData.index);
								if(intersects[ 0 ].object.userData.index){
									//console.log("index we got:" + intersects[ 0 ].object.userData.index);
									finishedDrift = false; hadClickedTimelineHotSpot = true;
		...
		function onDocumentMouseUp( event ) {
		       ...
		       // document.removeEventListener( 'mousemove', onDocumentMouseMove, false );
		        ...
		        if(grabbedTimeline){finishedDrift = false;}
		 		grabbedEarth = false;
		 		grabbedTimeline = false;
	...
	function addSkyBox(){
		...
		var mat = new THREE.MeshBasicMaterial({color: 0x7efaff, map: new THREE.TextureLoader().load("assets/images/stars.jpg") , side: THREE.BackSide, opacity:.1, transparent:true, blending:THREE.AdditiveBlending});
	...
	function addInnerRing(){
		...
		var mat = new THREE.MeshLambertMaterial({map: new THREE.TextureLoader().load("assets/images/inner_ring.png"), transparent:true, blending: THREE.AdditiveBlending, opacity:.01});
		...
		innerRing.show = function(){
			var tween = new TWEEN.Tween(this.material).to({ opacity:.9 }, 1000).start();
			tween.easing(TWEEN.Easing.Bounce.InOut);
			var tween = new TWEEN.Tween(this.material).to({ opacity:.9 }, 2000).start();
			tween.easing(TWEEN.Easing.Bounce.InOut);
		}
	...
	function addOuterRing(){
		...
		var mat = new THREE.MeshBasicMaterial({  map: new THREE.TextureLoader().load("assets/images/radial_layers_medium.jpg"), transparent:true, opacity:0 , blending:THREE.AdditiveBlending});
		...
		outerRing.show = function(){
			...
			scene.add(outerRing);
			var tween = new TWEEN.Tween(this.material).to({ opacity:.4 }, 1500).start();
...
/*
	...
	function addEarth(){
		...
		var mat = new THREE.MeshLambertMaterial({map: new THREE.TextureLoader().load("assets/images/earth.jpg")});
		...
		mat = new THREE.MeshLambertMaterial({transparent:true, color:0xff8340, blending:THREE.AdditiveBlending, opacity:.4, map: fire1 });
		...
		mat = new THREE.MeshLambertMaterial({transparent:true, color:0xff8340, blending:THREE.AdditiveBlending, opacity:.4, map: fire4 });
		...
		var mat = new THREE.MeshLambertMaterial({transparent:true, color:0x2AC7CC, blending:THREE.AdditiveBlending, opacity:.4, });
		...
		var mat = new THREE.MeshLambertMaterial({transparent:true, color:0x2AC7CC, blending:THREE.AdditiveBlending, opacity:.8, });
		var egh = new THREE.EdgesHelper( new THREE.Mesh(geo,mat), 0x2AC7CC );
		egh.material.linewidth = .5;
		egh.material.transparent = true;
		...
		var mat = new THREE.MeshLambertMaterial({blending:THREE.AdditiveBlending , transparent:true, color:0x2AC7CC, opacity:.6, map: new THREE.TextureLoader().load("assets/images/earth_political_alpha.png")});
		...
		for (i=0; i<180; i++){
        	...
        	if(i%10==0){
				...
				var pmat = new THREE.MeshPhongMaterial({map: new THREE.TextureLoader().load("assets/images/years/"+yr+".png"), transparent:true, opacity:1, side:THREE.DoubleSide});
		...
		ep2userData = {type:"earth"};
		earth.userData = {type:"earth"};
		earthPol.userData = {type:"earth"};
		earth.earthC1.userData = {type:"earth"};
		earth.earthC2.userData = {type:"earth"};
		earth.earthC3.userData = {type:"earth"};
		earth.earthC4.userData = {type:"earth"};
	...
	function addShips(){
		...
		// create ship mat
		var mat = new THREE.LineBasicMaterial( {  linewidth:1, color: 0x6FD5F0, transparent:true , opacity:.4} );

		// create ship mat
		var lasermat = new THREE.LineBasicMaterial( {  linewidth:1.5, color: 0x6FD5F0, transparent:true , opacity:.4} );
	...
	function addCityLines(){
		...
		IDR.lines = [];
		var lineMatBlue = new THREE.MeshBasicMaterial( { color: 0x3CD8FF , transparent:true, opacity:0.3,blending:THREE.AdditiveBlending} );
		var lineMatOrange = new THREE.MeshBasicMaterial( { color: 0xff8340 , transparent:true, opacity:0.3,blending:THREE.AdditiveBlending} );
		
	    var hexMat = new THREE.MeshBasicMaterial( { linewidth:2, color: 0xff8340 , transparent:true, opacity:0.5} );
		for(j=0;j<4;j++){
			for(i=0;i<36;i++){
		        ...
		        labelMat = new THREE.MeshBasicMaterial({side:THREE.DoubleSide, blending: THREE.AdditiveBlending, map: new THREE.TextureLoader().load("assets/images/city_labels/"+(j+1)+"_"+(i+1)+".png"), transparent:true , opacity:1});
	...
	function createHotSpot(){
		...
		var mat = new THREE.MeshBasicMaterial( {  blending: THREE.AdditiveBlending, color:0xff8340, transparent:true , opacity:.6} );
		...
		var mat = new THREE.LineBasicMaterial( {  blending: THREE.AdditiveBlending, linewidth:2, color:0xff8340, transparent:true , opacity:.8} );
		...
		var mat = new THREE.MeshBasicMaterial( {  blending: THREE.AdditiveBlending, color:0xff8340, transparent:true , opacity:.8, side: THREE.DoubleSide} );
		...
		var mat = new THREE.LineBasicMaterial( {  blending: THREE.AdditiveBlending, linewidth:2, color:0xff8340, transparent:true , opacity:.9} );
		...
		var mat = new THREE.LineBasicMaterial( {  blending: THREE.AdditiveBlending, linewidth:3, color:0xff8340, transparent:true , opacity:.5} );
		var geo = new THREE.Geometry();
	    geo.vertices.push(new THREE.Vector3(0,0,0));
		...
		hotspot.pMat = new THREE.MeshBasicMaterial( { color:0xff8340, blending: THREE.AdditiveBlending,  map: hotspot.texture,  side: THREE.DoubleSide , transparent:true} );
        ...
        var mat = new THREE.LineBasicMaterial( {blending: THREE.AdditiveBlending,  color:0xff8340 , transparent:true, opacity:.5} );
        ...
        var mat = new THREE.MeshBasicMaterial( { color:0xff8340 , transparent:true, opacity:0} );
        ...
        hotspot.show = function(){
        	if(!showingDetail){
	        	...
	        	if(IDR.mode!="explore"){
	        		trackEvent({'page':{'name':'Autoplay - '+ hotspot.event_data.title ,'path':'/autoplay/'+hotspot.event_data.title},'event':'vpv'});
	        	} else {
	        		trackEvent({'page':{'name':'Explore - '+ hotspot.event_data.title ,'path':'/explore/'+hotspot.event_data.title},'event':'vpv'});
	        	}
				...
				hotspot.texture = new THREE.Texture(hotspot.canvas);
				hotspot.pMat = new THREE.MeshBasicMaterial( {blending: THREE.AdditiveBlending,  map: hotspot.texture,  side: THREE.DoubleSide , transparent:true, color:0xff8340} );
	        	...
	        	var tween = new TWEEN.Tween(hotspot.scale).to({ x:1.7,y:1.7,z:1.7 }, 300).start();
				...
				setTimeout(function(){$("#preview").addClass("show");},150);
        ...
        hotspot.hide = function(){
        	var tween = new TWEEN.Tween(hotspot.scale).to({ x:.00001,y:.00001,z:.00001 }, 300).start();
			tween.easing(TWEEN.Easing.Back.InOut);
	...
	function createCloseButton(){
		...
		var mat = new THREE.MeshBasicMaterial( {  color: 0xFF9643, transparent:true , opacity:.3, side: THREE.DoubleSide} );
        var hexShape= new THREE.Shape();
		hexShape.moveTo(0,10);
		hexShape.lineTo(9,5);
		hexShape.lineTo(9,-5);
		...
		var geo = new THREE.ShapeGeometry( hexShape3 );
		var mat = new THREE.LineBasicMaterial( {  linewidth:2, color: 0xFF9643, transparent:true , opacity:.8} );
		...
		var mat = new THREE.MeshBasicMaterial( {  color: 0xFF9643, transparent:true , opacity:.7, side: THREE.DoubleSide} );
        var hexShape4 = new THREE.Shape();
		hexShape4.moveTo(0,10);
		hexShape4.lineTo(9,5);
		hexShape4.lineTo(9,-5);
		...
		var mat = new THREE.LineBasicMaterial( {  linewidth:2, color: 0xFF9643, transparent:true , opacity:.9} );
		var plus = new THREE.Shape();
		plus.moveTo(0,3);
		plus.lineTo(0,-3);
		plus.moveTo(0,0);
		...
		var mat = new THREE.LineBasicMaterial( {  linewidth:3, color: 0xFF9643, transparent:true , opacity:.7} );
		var geo = new THREE.Geometry();
	    geo.vertices.push(new THREE.Vector3(0,0,0));
	    geo.vertices.push(new THREE.Vector3(5, 0, 0));
		...
		*/
		...
		mat = new THREE.MeshBasicMaterial({side:THREE.DoubleSide, map: new THREE.TextureLoader().load("assets/images/close_text.png"), transparent:true , opacity:1});
		...
		// add listner classes
		hit.userData = {type:"close_target"};
		closeButton.hexSeg1.userData = {type:"close_target"};
		closeButton.hexSeg2.userData = {type:"close_target"};
		closeButton.hexSeg3.userData = {type:"close_target"};
		closeButton.hexSeg4.userData = {type:"close_target"};
		closeButton.plus.userData = {type:"close_target"};
		closeButton.hexSeg4.userData = {type:"close_target"};
		closeButton.title.userData = {type:"close_target"};
        ...
        closeButton.show = function(){
	      ...
	      //  closeButton.hex.material.opacity = 0;
	       ...
	       // var tween = new TWEEN.Tween(closeButton.hex.material).to({ opacity:1 }, 1000).delay(2000).start();
			...
			var tween = new TWEEN.Tween(closeButton.position).to({ y:yTarget }, 1000).delay(2000).start();

			var tween = new TWEEN.Tween(closeButton.rotation).to({  x:xrot  }, 1000).delay(2500).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut);

			tween.onComplete(function(){closeButton.lookAt(camera.position); });
        ...
        closeButton.hide = function(){
			...
			//tween.easing(TWEEN.Easing.Quadratic.InOut);
			var tween = new TWEEN.Tween(closeButton.position).to({ y:yTarget-300 },3000).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut);

			var tween = new TWEEN.Tween(closeButton.rotation).to({  x: (Math.PI/180) * -135 }, 1000).start();
	...
	function addDetailViewScreen(){
		...
		var mat = new THREE.MeshLambertMaterial({map: new THREE.TextureLoader().load("assets/images/detail_1_1.jpg"),opacity:.5, transparent:true,blending:THREE.AdditiveBlending});
		...
		var mat = new THREE.MeshLambertMaterial({color:0xff8340,opacity:.05, transparent:true,});
		viewScreenOverlay  = new THREE.Mesh(geo,mat);
		viewScreenOverlay.position.z=viewScreen2.position.z+1;

		var mat = new THREE.MeshBasicMaterial({color:0x7efaff,opacity:.05, transparent:true,});
	...
	function addTimeline(){
		...
		var mat = new THREE.LineBasicMaterial( {  linewidth:1, color: 0x6FD5F0, transparent:true , opacity:.7} );
		...
		for (i=0; i<360; i++){
        	...
        	if(i==1 || ((i-36)%12===0 && i > 36  && i <280)){
				...
				var pmat = new THREE.MeshPhongMaterial({map: new THREE.TextureLoader().load("assets/images/years/"+yr+".png"), transparent:true, opacity:1});
				p = new THREE.Mesh(pgeo,pmat);
			...
			if( i== 7 || i==19 || i==31 || i==43){
				...
				if(i!=43){
					...
					var pmat = new THREE.MeshPhongMaterial({map: new THREE.TextureLoader().load("assets/images/years/"+d+".png"), transparent:true, opacity:1});
					p = new THREE.Mesh(pgeo,pmat);
		...
		for (i=0;i<IDR.events.length;i++){
			...
			var hotspotMat = new THREE.LineBasicMaterial( { linewidth:2,color: 0xFF9643, transparent:true, opacity:.6 } );
			...
			mat = new THREE.MeshBasicMaterial({opacity:0, transparent:true});
			p = new THREE.Mesh(geo,mat);
			p.userData = {type:"timeline_hotspot",index: e.id};
			p.position.z=6;
			p.position.y=-1;
			p.position.x=0;
			...
			hotspot.userData = {type:"timeline_hotspot",index: e.id};
		...
		material = new THREE.MeshBasicMaterial( { transparent: true,opacity:0 , color: 0x6FD5F0} );
		...
		material = new THREE.MeshBasicMaterial( { transparent: true,opacity:.1 , color: 0x6FD5F0, side:THREE.DoubleSide} );
		circle = new THREE.Mesh( geo, material );
		timeline.outerDial.add( circle );
		geo = new THREE.CylinderGeometry(r+14,r+14,3,200,1, true);
		material = new THREE.MeshBasicMaterial( { transparent: true,opacity:.2 , color: 0x6FD5F0, side:THREE.DoubleSide} );
		circle = new THREE.Mesh( geo, material );
		timeline.outerDial.add( circle );
		
		// add some radial lines to this
		var lineGeo = new THREE.Geometry();
		mat = new THREE.LineBasicMaterial( { linewidth:2, transparent: true,opacity:.4 , color: 0x6FD5F0} );
		for (i=0; i<360; i+=4){
		...
		var mat = new THREE.LineBasicMaterial({linewidth:1,transparent: true,opacity:.3 , color: 0x6FD5F0});
		...
		s.lineTo(-.7,0);
		var geo = new THREE.ShapeGeometry(s);
		mat = new THREE.MeshLambertMaterial( { transparent: true,opacity:.6 , color: 0x7efaff , side:THREE.DoubleSide} );
		...
		var mat = new THREE.LineBasicMaterial({linewidth:2,transparent: true,opacity:.2 , color: 0x7efaff });
		...
		timeline.show = function(){
			...
			var tween = new TWEEN.Tween(timeline.position).to({y:ypos}, 3000).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut);
			setTimeout(function(){$("#date-content").addClass("show");},1000);
		...
		timeline.show2 = function(){
			...
			var tween = new TWEEN.Tween(timeline.position).to({y:ypos}, 2000).start();
		...
		timeline.hide = function(){
			var tween = new TWEEN.Tween(timeline.position).to({y:40}, 4000).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut);
	...
	function showDetailView(e){
		...
		if( !showingDetail){
			...
			if(IDR.mode=="explore"){
    			trackEvent({'page':{'name':'Explore  - '+ hotspot.event_data.title,'path':'/explore/'+ hotspot.event_data.title + '/view'},'event':'vpv'});
    		} else {
    			trackEvent({'page':{'name':'Autoplay  - '+ hotspot.event_data.title,'path':'/autoplay/'+ hotspot.event_data.title + '/view'},'event':'vpv'});
    		}

    		locked = true;
    		
    		// let animation complete then set state
    		setTimeout(function(){showingDetail = true;});
	    	...
	    	if(e.image.length>1 && !e.video){
	    		...
	    		window.detailGalleryTimer = setInterval(function(){
	    			...
	    			if(window.detailGalleryIndex==e.image.length){ window.detailGalleryIndex=0;}
					...
					if(window.detailGalleryAB==1){
						...
						var tween = new TWEEN.Tween(viewScreen.material.uniforms["opacity"]).to({"value":.4}, 1000).start();
						var tween = new TWEEN.Tween(viewScreen.material.uniforms["opacity"]).to({"value":.4}, 1000).start();
						var tween = new TWEEN.Tween(viewScreen3.material.uniforms["opacity"]).to({"value":0} , 1000).start();
						var tween = new TWEEN.Tween(viewScreen4.material.uniforms["opacity"]).to({"value":0}, 1000).start();
						window.detailGalleryAB = 0;
					} else {
						...
						viewScreen3.material.uniforms["opacity"].value = 0;
						viewScreen4.material.uniforms["opacity"].value = 0;
						var tween = new TWEEN.Tween(viewScreen3.material.uniforms["opacity"]).to({"value":.4}, 1000).start();
						var tween = new TWEEN.Tween(viewScreen4.material.uniforms["opacity"]).to({"value":.4}, 1000).start();
						var tween = new TWEEN.Tween(viewScreen.material.uniforms["opacity"]).to({"value":0}, 1000).start();
						var tween = new TWEEN.Tween(viewScreen2.material.uniforms["opacity"]).to({"value":0}, 1000).start();
			...
			var tween = new TWEEN.Tween(earthContainer.rotation).to({ x:0,y:0,z:0 }, 3000).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut);
			offset =  earth.orgRotY  - (Math.PI / (2 + hotspotYOffset ) * -1) + (Math.PI / (2) * -1);
			var tween = new TWEEN.Tween(earth.rotation).to( { x:(Math.PI/180) * 90 - (.15 *earth.orgRotY), y:offset }, 3000).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut);
			tween.onComplete(function(){  locked=false; targetEarthRotationX = earth.rotation.y;  targetEarthRotationY = earth.rotation.x});

			var tween = new TWEEN.Tween(camera.rotation).to({ x:(Math.PI/180) * -30 }, 3000).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut);
			var tween = new TWEEN.Tween(detailViewScreen.rotation).to({ x:(Math.PI/180) * -15 }, 3000).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut);
			var tween = new TWEEN.Tween(detailViewScreen.position).to({ z:-30,y:-225}, 3000).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut);

			// if mobile, portrait
			if(IDR.isPortrait){
				
				var tween = new TWEEN.Tween(detailViewScreen.position).to({ z:-120,y:-70}, 3000).start();
				tween.easing(TWEEN.Easing.Quadratic.InOut);
				var tween = new TWEEN.Tween(detailViewScreen.rotation).to({ x:(Math.PI/180) * -1 }, 3000).start();
				tween.easing(TWEEN.Easing.Quadratic.InOut);
				var tween = new TWEEN.Tween(camera.rotation).to({ x:(Math.PI/180) * -15 }, 3000).start();
				tween.easing(TWEEN.Easing.Quadratic.InOut);
			...
			setTimeout(function(e){$("#left-content").addClass("show");},1000);
			...
			tween.onComplete(function(){locked=false;});

			// pause speech
			if(IDR.mode=="play"){IDR.audio.pause("speech");}
			
			if(IDR.mode=="explore") { IDR.audio.playFromTo("speech", e.speechClipStart, e.speechClipEnd);}
	...
	function hideDetailView(){
		if(!locked){
    		...
    		//// hide the preview button
    		setTimeout(function(){$("#preview").addClass("show");},3500);
			...
			var tween = new TWEEN.Tween(earthContainer.rotation).to({ x:earthContainer.orgRotX,y:earthContainer.orgRotY,z:earthContainer.orgRotZ }, 3000).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut)
			// rotate back to start earth position
			var tween = new TWEEN.Tween(earth.rotation).to( { x:earth.orgRotX, y:earth.orgRotY, z: earth.orgRotZ }, 3000).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut);
			tween.onComplete(function(){ targetEarthRotationX = earth.rotation.y;  targetEarthRotationY = earth.rotation.x; });

			// detail screen back out
			var tween = new TWEEN.Tween(detailViewScreen.rotation).to({ x: detailViewScreen.orgRotX }, 4000).start();

			tween.easing(TWEEN.Easing.Quadratic.InOut);
			var tween = new TWEEN.Tween(scene.rotation).to({ x: 0,y:0,z:0 }, 3000).start();

			// roate camera back
			var tween = new TWEEN.Tween(camera.rotation).to({ x:0,y:0 }, 3000).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut)
			

			$("#left-content").removeClass("show");
			$("#right-content").removeClass("show");
			setTimeout(function(e){$("#left-content").css("display","none"); $("#right-content").css("display","none"); },2000);

			// unlock
			setTimeout(function(){showingDetail = false; locked = false;},3500);
	...
	function toggleSynopsis(){
		if(showingSynopsis){
			...
			if(IDR.isPortrait){
				var tween = new TWEEN.Tween(camera.rotation).to({ x:(Math.PI/180) * -15 }, 2000).start();
				tween.easing(TWEEN.Easing.Quadratic.InOut);
			}
			...
			if(IDR.isPortrait){
				var tween = new TWEEN.Tween(camera.rotation).to({ x:(Math.PI/180) * -30 }, 2000).start();
				tween.easing(TWEEN.Easing.Quadratic.InOut)
	...
	function playButtonClicked(){
		if(IDR.mode!="play"){
			...
			if( IDR.isPortrait ){ zpos = 700; } else { zpos = 550; }
			var tween = new TWEEN.Tween(camera.position).to({ z:zpos,y:0 }, 2000).start();
			tween.easing(TWEEN.Easing.Back.InOut);
	...
	function exploreButtonClicked(){
		if(IDR.mode!="explore"){
			IDR.mode = "explore";
			trackEvent({'page':{'name':'Explore','path':'/explore'},'event':'vpv'});
			$("#play-button").addClass("show");
			$("#explore-button").removeClass("show");
			//$("#play-button").css("z-index",1);
			//$("#explore-button").css("z-index",0);
			if( IDR.isPortrait ){ zpos = 700-20; } else { zpos = 550-20; }
			var tween = new TWEEN.Tween(camera.position).to({ z:zpos,y:-8 }, 2000).start();
			tween.easing(TWEEN.Easing.Back.InOut);
    ...
    function rotateToLatLng(lat,lng, offsetY){
    	if(!showingDetail){
			...
			TWEEN.removeAll();
			var tween = new TWEEN.Tween(earth.rotation).to( { x:newRotX, y:newRotY, z:newRotZ }, 500).start();
			tween.easing(TWEEN.Easing.Quadratic.InOut);
			tween.onComplete(function(){  locked=false; targetEarthRotationX = earth.rotation.y;  targetEarthRotationY = earth.rotation.x});
			//
			//locked=false;
    ...
    function playProgress(){
    	...
    	if(IDR.mode == "play" && IDR.speechPlaying ){
    		...
    		var index = 0;
    		if(hotspot.event_data){index = hotspot.event_data.id;}
    ...
    window.playSpeech = function(){
    	if(IDR.mode=="play"){
    		...
    		if(!hotspot.event_data) { currentIndex = 0; } else {currentIndex = hotspot.event_data.id -1;} 
    ...
    function getHotSpotRot(index){
    	if(index==IDR.events.length){index=IDR.events.length-1;}
    ...
    function start(){
		...
		$("#instructions").css("z-index","350");
		setTimeout(function(){$("#instructions_text").animate({opacity:.8, "padding-top":0},2000);},2000);
		setTimeout(function(){$("#instructions_thumb1").animate({opacity:.8 ,"top":"45%"},1000);},3000);
		setTimeout(function(){$("#instructions_thumb2").animate({opacity:.8 ,"top":"45%"},1000);},4000);
		setTimeout(function(){$("#instructions_thumb3").animate({opacity:.8 ,"top":"45%"},1000);},5000);

		setTimeout(function(){$("#instructions").animate({opacity:0},1000);},10000);
		setTimeout(function(){$("#instructions").css("z-index","0");},11000);

		setTimeout(function(){IDR.audio.play("ambient");},1000);


		if(IDR.deepLinkID==21 ) { IDR.deepLinkID=0;}
		...
		setTimeout(function(){
    		...
    		if(IDR.deepLinkID>0){
	    		...
	    		var posX = camera.position.x; var posY = camera.position.y; var posZ = camera.position.z; 
				var rotX = camera.rotation.x; var rotY = camera.rotation.y; var rotZ = camera.rotation.z; 
				camera.rotation.z = Math.PI/180 * -50;
				camera.rotation.y = Math.PI/180 * -90;
				camera.rotation.x = Math.PI/180 * -30;
		    	var tween = new TWEEN.Tween(camera.rotation).to({ x:rotX, y:rotY, z:rotZ}, 5000).start();
				tween.easing(TWEEN.Easing.Quadratic.InOut);
				var posX = camera.position.x; var posY = camera.position.y; var posZ = camera.position.z; 
				camera.position.set(-500,700,-250);
				var tween = new TWEEN.Tween(camera.position).to({ x:posX, y:posY, z:posZ}, 5000).start();
				tween.easing(TWEEN.Easing.Quadratic.InOut);
				timeline.hide2();

				setTimeout(function(){ outerRing.show(); },10000);
				setTimeout(function(){IDR.audio.play("ambient");},1000);
				setTimeout(function(){ timeline.show();},3000);
				setTimeout(function(){ 
					locked = false; finishedDrift =false; hadClickedTimelineHotSpot = true;
					hotspot.event_data = IDR.events[IDR.deepLinkID-1];
					IDR.showingEventIndex = IDR.deepLinkID-1;
					hotspot.show();
					if(IDR.deepLinkID==2) {hotspot_alt1.show();}
					setTimeout(function(){
						timeline.dial.rotation.y = getHotSpotRot(IDR.deepLinkID-1);
						targetRotationX = -getHotSpotRot(IDR.deepLinkID-1);
					},500);

				 	trackEvent({'ea':{'name':'Nav','action':'Timeline','label':'TIMELINE'},'event':'se'});
				 	$("#play-button").addClass("show");
					$("#explore-button").removeClass("show");
				},6000);
    		} else {
    		// regular site experience
	    		...
	    		IDR.mode = "play"; 
	    		if(IDR.isMobile) { IDR.mode="explore";}
	    		var posX = camera.position.x; var posY = camera.position.y; var posZ = camera.position.z; 
				var rotX = camera.rotation.x; var rotY = camera.rotation.y; var rotZ = camera.rotation.z; 
				camera.rotation.z = Math.PI/180 * -20;
				camera.rotation.y = Math.PI/180 * -50;
				camera.rotation.x = Math.PI/180 * 20;
		    	var tween = new TWEEN.Tween(camera.rotation).to({ x:rotX, y:rotY, z:rotZ}, 6000).start();
				tween.easing(TWEEN.Easing.Quadratic.InOut);
				...
				setTimeout(function(){if(!IDR.isMobile){playSpeech();} },8500);
			
				
				//outerRing.show(); innerRing.show();
				setTimeout(function(){ timeline.show();},6500);
				setTimeout(function(){ 
					$("#explore-button").addClass("show"); 
